...
public class Accent extends JDialog implements Base{
	...
	JPanel p3 = get(new JPanel(new GridLayout(0, 1, 0, 30)));
	JPanel p4 = get(new JPanel(new GridLayout(0, 1, 0, 30)));
	JPanel p5 = get(new JPanel(new GridLayout(1, 2, 5, 0)));
	
	JButton btn1 = get(new JButton("수정"));
	JButton btn2 = get(new JButton("취소"));
	
	JLabel lab1 =get(new JLabel("계정"), set(25));
	JLabel lab;
	String st[] = "id, pwd, name, email, point".split(", ");
	...
	public Accent() {
		...
		SetDial(this, "계정", DISPOSE_ON_CLOSE, 400, 500);
		design();
		action();
		setVisible(true);
	...
	public void design() {
		...
		txt1 = get(new JTextField() {
			protected void paintComponent(Graphics g) {
				...
				g.drawString(member.get(0).get(1), this.getInsets().left, this.getInsets().top + g.getFontMetrics().getMaxAscent() + 10);
		...
		txt5 = get(new JTextField() {
			protected void paintComponent(Graphics g) {
				...
				g.setColor(Color.black);
				g.drawString(member.get(0).get(5), this.getInsets().left, this.getInsets().top + g.getFontMetrics().getMaxAscent()+ 10);
		...
		for (int i = 0; i < st.length; i++) {
			p3.add(lab = get(new JLabel(st[i])));
		}
	...
	public void action() {
		...
		btn1.addActionListener(e->{
			...
			if (txt2.getText().isBlank() || txt3.getText().isBlank() || txt4.getText().isBlank()) {
				...
				Update("update user set pwd = ?, name = ?, email = ? where no = ?", txt2.getText(), txt3.getText(), txt4.getText(), member.get(0).get(0));
