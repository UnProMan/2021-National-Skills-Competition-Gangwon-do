...
public class Login extends JFrame implements Base{
	...
	JPanel p1 = get(new JPanel(new BorderLayout(5, 5)), set(new EmptyBorder(100, 40, 40, 30)));
	JPanel p2 = get(new JPanel(new BorderLayout(5,0)));
	JPanel p3 = get(new JPanel(new BorderLayout()));
	JPanel p4 = get(new JPanel(new GridLayout(2, 1, 0, 5)));
	JPanel p5 = get(new JPanel(new BorderLayout()), set(new EmptyBorder(30, 40, 60, 0)));
	
	JLabel lab1 = get(new JLabel("로그인"), set(20));
	JLabel lab2 = get(new JLabel("아이디/비밀번호 찾기"));
	JLabel lab3 = get(new JLabel("새로운 계정 만들기 →"));
	JLabel img = getimg(file("login.jpg"), 500, 400);
	
	JTextField txt1 = gettxt(new JTextField(), "Id");
	JPasswordField txt2 = getpass(new JPasswordField(), "Password");
