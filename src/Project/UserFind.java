...
public class UserFind extends JDialog implements Base{
	...
	JPanel p1 = get(new JPanel(new GridLayout(0, 1, 0, 15)), set(new EmptyBorder(30, 40, 30, 40)));
	
	JLabel lab1 = get(new JLabel("아이디 찾기"), set(25));
	JLabel lab2 = get(new JLabel("비밀번호 찾기"), set(25));
	
	JTextField txt1 = gettxt(new JTextField(), "Name");
	JTextField txt2 = gettxt(new JTextField(), "E-mail");
	JTextField txt3 = gettxt(new JTextField(), "Name");
	JTextField txt4 = gettxt(new JTextField(), "Id");
	JTextField txt5 = gettxt(new JTextField(), "E-mail");
	
	JButton btn1 = get(new JButton("게속"));
	...
	public UserFind() {
		...
		SetDial(this, "아이디/비밀번호 찾기", DISPOSE_ON_CLOSE, 450, 550);
		design();
		action();
		setVisible(true);
