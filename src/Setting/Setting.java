...
public class Setting implements Base{
	...
	public Setting() {
		...
		try {
			...
			s.executeUpdate("create table schedule(no int primary key auto_increment, departure_location2_no int not null, arrival_location2_no int not null, date datetime not null, elapsed_time time not null, foreign key(departure_location2_no) references location2(no), foreign key(arrival_location2_no) references location2(no))");
			...
			s.executeUpdate("create table recommend_info(recommend_no int not null, title varchar(50) not null, descrption varchar(1000), img longblob not null, foreign key(recommend_no) references recommend(no))");
			...
			Query("select * from recommend_info ri, recommend r, location l where ri.recommend_no = r.no and r.location_no = l.no;", list);
			
			for (int i = 0; i < list.size(); i++) {
				
				String st1 = list.get(i).get(7).contentEquals("부산") ? "busan" : list.get(i).get(7).contentEquals("강원도") ? "gangwondo" : list.get(i).get(7).contentEquals("광주") ? "gyeongju" : list.get(i).get(7).contentEquals("서울") ? "seoul" : "jeollanam-do";
				String file = file("recommend/" + st1 + "/" + list.get(i).get(1) + ".jpg");
				Saveimg(file, list.get(i).get(0), list.get(i).get(1));
				
			}
			
			JOptionPane.showMessageDialog(null, "셋팅");
